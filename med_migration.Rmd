...
fwrite(missing_med, "output/missing_med_cleaned.csv")
missing_med <- data.frame(fread("output/missing_med_cleaned_manual.csv"))
```
...
```{r, echo=FALSE}
fwrite(summary_all_month, "output/missing med final.csv")
fwrite(summary_all_month, "output/summary data by month.csv")
fwrite(summary_all_year, "output/summary data by year.csv")
```
...
summary_all_month %>% 
  ...
  geom_col(na.rm = TRUE) +
  facet_grid(cols = vars(Year), rows = vars(Migration.Route)) +
